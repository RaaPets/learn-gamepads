...
pub struct CellsSpace<T: Default + Clone> {
    pub width: usize,
    pub height: usize,
    cells2d: Box<[Box<[T]>]>,
}

impl<T: Default + Clone> CellsSpace<T> {
    pub fn new(width: usize, height: usize) -> Self {
        Self {
            width,
            height,
            cells2d: vec![vec![T::default(); width].into_boxed_slice(); height].into_boxed_slice(),
        }
    }
...
impl<T: Default + Clone> std::ops::Index<(usize, usize)> for CellsSpace<T> {
    type Output = T;
...
impl<T: Default + Clone> std::ops::IndexMut<(usize, usize)> for CellsSpace<T> {
...
impl<T: Default + Clone> std::ops::Index<(isize, isize)> for CellsSpace<T> {
    type Output = T;
...
impl<T: Default + Clone> std::ops::IndexMut<(isize, isize)> for CellsSpace<T> {
...
mod cells_world {
    ...
    fn normalize_example() {
        let width = 3;
        let height = 5;
        let mut world = CellsSpace::<CellState>::new(width, height);
    ...
    fn normalize_ij_3() {
        let width = 3;
        let height = 3;
        let world = CellsSpace::<CellState>::new(width, height);
